...
def load_data():
    ...
    # Drop rows with missing country or AQI value for stability
...
def show_country_pollutants(df):
    ...
    # Drop NaNs first to avoid TypeError: float vs str
    df_clean = df.dropna(subset=["country"]).copy()

    # Create sorted list of clean text-only countries
    ...
    # Select pollutant columns dynamically
